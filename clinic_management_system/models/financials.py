...
class Financials(models.Model):
    ...
    _name = "syscraft.clinic.financials.tbl"
    ...
    bill_items_ids = fields.One2many('syscraft.bill.items.for.bill.tbl', 'bill_bill_id')
    ...
    def action_open_patient(self):
        for rec in self:
            if rec.PATIENT_id:
                ...
                raise ValidationError("Name value is empty cannot get patient details ")


class BillItemsForBills(models.Model):
    _name = "syscraft.bill.items.for.bill.tbl"
    _description = "Bill Bill Items"
    ...
    bill_bill_id = fields.Many2one('syscraft.clinic.financials.tbl')
